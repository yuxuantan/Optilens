...
def find_lowest_bear_trap_within_price_range(potential_bear_traps, high_point_date, low_price, high_price):
    valid_traps = [trap for trap in potential_bear_traps if trap[0] < high_point_date and low_price < trap[1] < high_price]
    return min(valid_traps, key=lambda x: x[1]) if valid_traps else None
